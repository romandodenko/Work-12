...
if (document.body.clientWidth < 992) {
  if (roomsSlider) {
    const roomsSwiper = new Swiper(roomsSlider, {
      observer: true,
      observeParents: true,
      watchOverflow: true,
      slidesPerView: 1,
...
const roomsSliderSlide = document.querySelectorAll(".rooms-slider__slide");

if (roomsSliderSlide) {
  roomsSliderSlide.forEach(function (e, i) {
    const roomsSliderSlideIndex = e.querySelector(".rooms-slider__num");
    if ((i + 1) < 10) {
      roomsSliderSlideIndex.innerHTML = "0" + (i + 1);
    } else {
      roomsSliderSlideIndex.innerHTML = i + 1;
    }
  })
}