...
const footerBottomLeft = document.querySelector(".footer__bottom-left");
...
if (footerBottomLeft) {
  if (navFooter) {
    if (document.body.clientWidth < 992) {
      footerBottomLeft.append(navFooter);
    }
  }
}

const diffSlider = document.querySelector(".diff-slider");

if (diffSlider) {
  const diffSwiper = new Swiper(diffSlider, {
    observer: true,
    observeParents: true,
    watchOverflow: true,
    slidesPerView: 'auto',
    ...
    pagination: {
      el: '.diff-slider-pagination',
      clickable: true,
    },
    navigation: {
      nextEl: '.diff-slider-next',
      prevEl: '.diff-slider-prev',
    },
  });
}